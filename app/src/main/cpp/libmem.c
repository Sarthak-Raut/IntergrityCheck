...
#		if LM_OS == LM_OS_LINUX || LM_OS == LM_OS_ANDROID
		LM_SNPRINTF(maps_path, LM_ARRLEN(maps_path),LM_STR("%s/%d/maps"), LM_PROCFS, proc.pid);
#		elif LM_OS == LM_OS_BSD
		LM_SNPRINTF(maps_path, LM_ARRLEN(maps_path),LM_STR("%s/%d/map"), LM_PROCFS, proc.pid);
#		endif
	...
	case LM_MOD_BY_STR:
		{
			...
			if (pathlen >= parg->len) {
				if (!LM_STRCMP(&path[pathlen - parg->len],modstr)) {
					*(parg->modbuf) = mod;
					return LM_FALSE;
				}
			}
